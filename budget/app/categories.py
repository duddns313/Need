...
CATEGORIES = {
    ...
    '구독료': FIXED,
    '대출이자': FIXED,
    # 주담대 원리금. 이자와 원금이 한 번에 빠져나가는데 명세엔 합계만 남는다.
    # 원금 몫은 집 지분이 되어 순자산을 늘리지만, 매달 반드시 나가야 하는
    # 돈이라는 점에서는 고정비다. 가계 예산에서는 고정비로 세고, 원금이
    # 얼마인지는 「자산」 탭의 대출 잔액이 따로 보여준다.
    '대출원리금': FIXED,
    # 변동비 — 먹는 돈을 넷으로 쪼갠다.
    ...
    '대신결제': EXCLUDED,
    # 회사가 관리하는 연구비·프로젝트 계좌. 들어온 만큼 그대로 나가는
    # 통로일 뿐 내 살림이 아니다. 계좌째로 뺀다.
    '연구비계좌': EXCLUDED,
    # 공제·적금이 만기되어 돌아온 목돈. 그동안 내가 부은 돈이 돌아온 것이라
    # 그 달에 번 돈으로 세면 소득도 저축률도 통째로 거짓이 된다.
    '만기수령': EXCLUDED,
}
