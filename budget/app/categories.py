...
CATEGORIES = {
    ...
    '개인송금': VARIABLE,
    # 가족에게 보낸 돈(용돈 등). 배우자·본인 이체(제외)와 다르다 — 그건 내
    # 계좌 사이를 옮긴 것이지만 이건 실제로 집 밖으로 나가는 돈이다.
    '가족·용돈': VARIABLE,
    '여행·숙박': VARIABLE,
    '자동차': VARIABLE,
    '교육': VARIABLE,
    ...
    # 같이 먹고 나눠 낸 돈이 사람 이름으로 들어온 것. 번 게 아니다.
    '정산받음': EXCLUDED,
    # 아이디를 같이 쓰는 가족 대신 긁어 주고 그 자리에서 돌려받은 것.
    # 결제와 입금이 짝을 이루므로 둘 다 빼야 앞뒤가 맞는다. 한쪽만 빼면
    # 쓴 돈이나 번 돈 한쪽이 통째로 부풀려진다.
    '대신결제': EXCLUDED,
}
